module goomba_right_rom
	...
	always @(*) begin
		...
		if(({row_reg, col_reg}>=10'b1010111100) && ({row_reg, col_reg}<10'b1011001000)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1011001000) && ({row_reg, col_reg}<10'b1011011000)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1011011000) && ({row_reg, col_reg}<10'b1011011100)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1011011100) && ({row_reg, col_reg}<10'b1011101000)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1011101000) && ({row_reg, col_reg}<10'b1011111000)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1011111000) && ({row_reg, col_reg}<10'b1011111100)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1011111100) && ({row_reg, col_reg}<10'b1100000110)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1100000110) && ({row_reg, col_reg}<10'b1100001010)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1100001010) && ({row_reg, col_reg}<10'b1100010110)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1100010110) && ({row_reg, col_reg}<10'b1100011110)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1100011110) && ({row_reg, col_reg}<10'b1100100110)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1100100110) && ({row_reg, col_reg}<10'b1100101010)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1100101010) && ({row_reg, col_reg}<10'b1100110110)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1100110110) && ({row_reg, col_reg}<10'b1100111110)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1100111110) && ({row_reg, col_reg}<10'b1101000110)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1101000110) && ({row_reg, col_reg}<10'b1101001100)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1101001100) && ({row_reg, col_reg}<10'b1101010100)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1101010100) && ({row_reg, col_reg}<10'b1101011110)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1101011110) && ({row_reg, col_reg}<10'b1101100110)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1101100110) && ({row_reg, col_reg}<10'b1101101100)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1101101100) && ({row_reg, col_reg}<10'b1101110100)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1101110100) && ({row_reg, col_reg}<10'b1101111110)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1101111110) && ({row_reg, col_reg}<10'b1110001000)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1110001000) && ({row_reg, col_reg}<10'b1110001110)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1110001110) && ({row_reg, col_reg}<10'b1110010010)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1110010010) && ({row_reg, col_reg}<10'b1110011100)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1110011100) && ({row_reg, col_reg}<10'b1110101000)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1110101000) && ({row_reg, col_reg}<10'b1110101110)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1110101110) && ({row_reg, col_reg}<10'b1110110010)) color_data = 12'b111111001011;
		if(({row_reg, col_reg}>=10'b1110110010) && ({row_reg, col_reg}<10'b1110111100)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1110111100) && ({row_reg, col_reg}<10'b1111001010)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1111001010) && ({row_reg, col_reg}<10'b1111010000)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1111010000) && ({row_reg, col_reg}<10'b1111010010)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1111010010) && ({row_reg, col_reg}<10'b1111011010)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1111011010) && ({row_reg, col_reg}<10'b1111101010)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1111101010) && ({row_reg, col_reg}<10'b1111110000)) color_data = 12'b010000100001;
		if(({row_reg, col_reg}>=10'b1111110000) && ({row_reg, col_reg}<10'b1111110010)) color_data = 12'b000000000000;
		if(({row_reg, col_reg}>=10'b1111110010) && ({row_reg, col_reg}<10'b1111111010)) color_data = 12'b010000100001;

		if(({row_reg, col_reg}>=10'b1111111010) && ({row_reg, col_reg}<=10'b1111111111)) color_data = 12'b000000000000;
	end
endmodule